...
/**
 * A given-name, optional but solitary middle name, and family-surname triplet.
 * 
 * @author James Pede
 * @version Jun1'14
 */
public class Name
{
    ...
    public String getInitials()
    {
        return (first.charAt(0)
                + (null == middle ? "" : INITIALS_DELIMITER + middle.charAt(0))
                + INITIALS_DELIMITER + last.charAt(0)
                + INITIALS_DELIMITER
            ).toUpperCase();
    } // end method
    ...
    {
        // This validation isn't necessary due to the restrictions on passed parameters.
        //if (null == impromptuNoun || 0 == impromptuNoun.length())
        //    return impromptuNoun;
        
        return impromptuNoun.substring(0, 1).toUpperCase() + impromptuNoun.substring(1).toLowerCase();
    } // end method
    ...
    {
        return getFirstName()
            + (null == middle ? "" : PRETTY_DELIMITER + getMiddleName())
            + PRETTY_DELIMITER + getLastName();
    } // end method
    ...
    public void set(String first, String middle, String last)
    {
        ...
        for (String anyName : new String[] {first, middle, last})
            if (null == anyName
                || MINIMUM_NAME_LENGTH > (nameLength = anyName.length())
